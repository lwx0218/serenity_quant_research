...
public partial class AccountPage(ITwoLevelCache cache, ITextLocalizer localizer,
    IConfiguration configuration) : Controller
{
    ...
    protected IConfiguration Configuration { get; } = configuration ?? throw new ArgumentNullException(nameof(configuration));

    [HttpGet]
    public ActionResult Login(int? denied, string activated, string returnUrl)
    {
        if (Configuration.GetValue<bool>("OpenAccess:Enabled"))
            return Redirect("~/");
